...
| [SCHWAB_STOCKS_DESIGN](dawn/archive/SCHWAB_STOCKS_DESIGN.md) | Charles Schwab stocks tool (read-only): quotes, portfolio + unrealized P/L, price history/analytics, fundamentals, and recent transactions. Account-hash indirection, a leg-and-sign transaction classifier pinned from a live-capture spike of the undocumented schema (dividend payer only in prose, fees as legs), account/memo PII masking, and the deferred MWR/daily-snapshot follow-ups. OAuth via the shared client; weekly re-link |
| [MUSIC_SEARCH_REDESIGN](dawn/archive/MUSIC_SEARCH_REDESIGN.md) | Music search redesign: genre/year tag extraction (local + Plex) with a genre-erasure fix, and a relevance-ranked structured query core (music_db_query + pure music_rank scorer) shared by every surface. Breadth-by-default free text vs word-boundary fielded artist/title precision; path dropped from matching. Shelved-with-triggers: FTS5 (Phase 2b), semantic mood (Phase 4), browser filter UI (Phase 3) |
