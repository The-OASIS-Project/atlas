...
Short-term focus: l1_microbench promotion + legacy-data filter scan (~agent 1 hr · api ~$2 · 2 ckpt). Provenance Phase A + B + budget probe are all closed; 12288 is the operational sweet spot. After short-term, the medium-term roadmap (priority-ordered for the JARVIS goal): **speaker mis-attribution → dynamic context injection → multi-language injection filter → "I don't remember" gate**.
...
**For the next session: load `atlas/dawn/memory/STATE.md` + `atlas/dawn/memory/PROVENANCE.md`. Phase B is shipped; item #6 is closed (no orchestrator drift); item #5 is closed (24576 sweet spot is at 12288 — diminishing returns confirmed).  Phase A budget-sweep is fully settled.  Next high-value items are #7 (l1_microbench promotion, ~30m) and #8 (legacy-data filter scan, ~30m); after that the medium-term roadmap, JARVIS-goal-ordered: speaker mis-attribution → dynamic context injection → multi-language injection filter → "I don't remember" gate.**
