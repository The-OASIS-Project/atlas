...
| [RERANKER_HEURISTIC.md](RERANKER_HEURISTIC.md) | shipped (May 2026, commit `0e9d5c3`) | Non-ML dominant-token over-inclusion heuristic that closes the cross-encoder reranker TODO.  Three sub-phases ran same-day 2026-05-13: Phase A diagnostic (3 severe-pathology probe cases unanimous FAIL at HEAD), Phase B-i Python prototype (parameter-robustness grid + 18/18 quorum at canonical params), Phase B-ii C implementation in `src/memory/focus_dominant_token.{c,h}` (~280+105 LOC + 13 Unity tests).  Detects low-IDF dominant query tokens in the candidate pool, penalizes candidates whose only query-overlap is via those tokens, mutates `semantic_score` in the working pool so the existing composite formula propagates the penalty.  Default-on; three config keys at `[memory.focus_injection.dominant_token_heuristic]`.  Total ~$1.08 paid, agent ~8 hours. |
