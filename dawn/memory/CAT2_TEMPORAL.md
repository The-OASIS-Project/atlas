...
- **Dialog [D1:8]** (session_1 in dataset, NOT session 4 as previously noted): Jolene says *"This pendant reminds me of my mother, she gave it to me in 2010 in Paris."*  Verified against `~/datasets/locomo/data/locomo10.json` 2026-05-07 — first speaker in conv 7 is Deborah (role=user), Jolene is role=assistant.
...
- **Update 2026-05-07: Prompt-only fix attempt #1 reverted.**  Tried opener reframe ("extract user information" → "extract information about the people who participated") + new "DERIVE subject from speaker, not role" guideline.  Full 10-conv LoCoMo bench at budget=12288: overall `recall_generation` -0.8pp (0.493 → 0.485), **cat-2 -3.7pp (0.511 → 0.474)**, **cat-1 -2.5pp (0.404 → 0.379)**, cat-3 -2.2pp, cat-4 -1.6pp, cat-5 +4.3pp.  Reach behaved as expected for a generation-side change (within ±2pp on each cat, +0.6pp overall).  Stop conditions hit on both cat-1 (>1pp regression) and cat-2 (negative), so reverted.  Suspected mechanism: top-K crowding analogous to Phase 1 → 1.1.  The reframed opener and new bullet caused the LLM to extract more facts from the assistant-role side, diluting the existing canonical-subject signal in the top-20 retrieval window.  Future attempt should avoid widening fact coverage and instead target attribution post-hoc — e.g., a second-pass validator that checks fact subjects against turn speakers, or a structural change that separates "extract about user" from "extract about other participants" so the two streams don't compete for top-K.  Diagnostic note: `/tmp/speaker_misattr_diag.md`.  Bench artefacts: `/tmp/speaker_fix_full/full_postfix.json` (aggregate), `full_run.log` (per-conv), `judgements.json`/`generations.json`/`correctness.json` (per-QA verdicts; opaque hash keys, not directly grepable for Case 10).
- **Update 2026-05-08: Attempt #2 (cγ — speaker-named JSON structural reframe) closed, no production ship.**  Goal was to fix the `role=user → extract user info` prior structurally rather than via prompt-additive instructions.  Mechanism: replace `{role:user/assistant, content}` with `{role:user, content, speaker:Name}` so the role-binary signal carries no information — the LLM gets the speaker name as a separate field and never sees a role binary it can map to "the user."  Probe results across 3 models (Anthropic / OpenAI / local Qwen) **passed cleanly**: +25pp aggregate c3 lift (8/12 + 5/12 + 8/12 vs HEAD 6/1/5) with **0% c1 widening** and clean c2 — exactly the structural fix shape attempt #1 lacked.  Wired into `bench_memory_pipeline.c` (bench-only; production `memory_extraction.c` untouched) and ran partial full LoCoMo at budget=12288.  Result at 8/10 convs: `recall_generation 0.495` vs HEAD 0.493 = **~0pp lift**; killed early at the same trajectory as attempt #1's curve, statistically indistinguishable from baseline.  Case 10 specifically: extraction now correctly attributes "Jolene owns a pendant given to her by her mother in 2010 in Paris" — the per-case fix works.  But the per-fact attribution effect doesn't move LoCoMo's 1982-question full-bench aggregate.  **Load-bearing finding: probe-vs-bench gap quantified for this class of fix (+25pp probe c3 → ~0pp full bench).** Speaker mis-attribution at extraction time is real and probe-detectable but is dominated by retrieval / top-K dynamics in full-conv extraction.  Permanent infrastructure shipped: `benchmarks/bench_speaker_attribution.py` (multi-model + 2-of-3 quorum + Component 3 fix-rate mini-bench).  Speaker mis-attribution priority demoted in STATE.md medium-term — defer until DAWN expands to multi-party scenarios, production telemetry shows attribution failures, or a fundamentally different fix shape emerges (post-extraction validator using provenance triples is the next logical attempt if revisited).  Total cost across attempts #1 + #2 + probe build: ~$13.
